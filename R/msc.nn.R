                    ...
                    nLevels, type = 1, smooth = FALSE, eps=0.01)
{
    ...
    if(missing(nLevels) || is.null(nLevels)){
       ...
       mscl[[1]] <- structure(
            list(call=call,
              pLevel=-1,
              partition = as.vector(res[[2]]),
              ascending = as.vector(res[[5]]),
              maxs=as.vector(res[[3]]),
              mins=as.vector(res[[4]])
            ),
            class = "msc.level")
      ...
      for(i in 0:(nLevels-1)){
        ...
        msc <- structure(
                list(call=call,
                  pLevel=p,
                  partition = as.vector(res[[i*4+2]]),
                  ascending = as.vector(res[[i*4+5]]),
                  maxs = as.vector(res[[i*4+3]]),
                  mins=as.vector(res[[i*4+4]])
                ),
                class = "msc.level")
        msc$partitionSize <- c()
